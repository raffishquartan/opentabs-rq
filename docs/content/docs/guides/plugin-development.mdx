...
- **`version`** — plugin version string (e.g., semver `"0.0.1"`). Used for display purposes.
...
Beyond tools, your plugin can also declare `resources` (read-only data the agent can browse) and `prompts` (reusable message templates). Both require the `override` keyword because they are non-abstract properties declared on the base class — `tools` is abstract and does not require `override`. See [Resources & Prompts](/docs/guides/resources-prompts) for the full guide.

Similarly, lifecycle hooks (`onActivate`, `onDeactivate`, `onNavigate`, `onToolInvocationStart`, `onToolInvocationEnd`) require `override` because they are optional methods defined on the base class (TypeScript's `noImplicitOverride` rule enforces this). See [Lifecycle Hooks](/docs/sdk/lifecycle-hooks) for usage examples.
