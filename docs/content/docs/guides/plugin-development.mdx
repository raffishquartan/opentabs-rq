...
```bash
npx create-opentabs-plugin my-app --domain .example.com --display "My App"
cd my-app
npm install
```
...
- `fetchJSON(url, init?, schema?)` — GET/DELETE without a body
- `postJSON(url, body, init?, schema?)` — POST/PUT/PATCH with a body (same for `putJSON`, `patchJSON`, `deleteJSON`)
