...
Beyond tools, your plugin can also declare `resources` (read-only data the agent can browse) and `prompts` (reusable message templates). Both require the `override` keyword because they are optional properties with default values on the base class — `tools` is abstract and does not require `override`. See [Resources & Prompts](/docs/guides/resources-prompts) for the full guide.
