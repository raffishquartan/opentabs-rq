...
1. **Runtime** — verifies Node.js 20+ is installed
...
Delete the config file. The server regenerates a fresh config (with a new secret) on the next `opentabs start`.
...
Since the server secret is stored in the config file, resetting config means your MCP client configuration will need to be updated with the new secret after the next `opentabs start`.
...
If the MCP server is running, it is notified using the old secret so the transition is seamless on the server side. The Chrome extension must be reloaded to pick up the new secret from `auth.json`.

```bash
# Rotate the secret
opentabs config rotate-secret

# Then reload the Chrome extension from chrome://extensions/
```
