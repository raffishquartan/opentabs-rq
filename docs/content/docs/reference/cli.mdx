...
1. **Runtime** — verifies Node.js 22+ is installed
...
9. **MCP client config** — checks Claude Code and Cursor MCP configuration files for correct server entries
...
```bash
npx create-opentabs-plugin my-plugin --domain .example.com --display "My App"
```
...
<Table>
  ...
  <Table.Body>
    ...
    <Table.Row>
      ...
      <Table.Cell>Display the full authentication secret instead of masking it.</Table.Cell>
    </Table.Row>
  </Table.Body>
</Table>
