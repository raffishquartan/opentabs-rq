...
const DialogContent = ({ className, children, ref, ...props }: ComponentProps<typeof DialogPrimitive.Content>) => (
  <DialogPrimitive.Portal>
    ...
    <DialogPrimitive.Content
      ref={ref}
      className={cn(
        'data-[state=open]:fade-in-0 data-[state=open]:zoom-in-95 data-[state=closed]:fade-out-0 data-[state=closed]:zoom-out-95 fixed top-1/2 left-1/2 z-50 grid max-h-[85vh] w-full max-w-sm -translate-x-1/2 -translate-y-1/2 overflow-hidden rounded border-2 border-border bg-background shadow-md duration-200 data-[state=closed]:animate-out data-[state=open]:animate-in',
        className,
      )}
      {...props}>
      <VisuallyHidden>
      ...
      {/* min-h-0/min-w-0 let the body shrink and scroll internally instead of the
          dialog growing past max-h-[85vh] or wider than max-w-sm and taking the
          footer's action buttons off screen with it. */}
...
const DialogHeader = ({ className, children, ref, ...props }: ComponentProps<'div'>) => (
  <div
    ref={ref}
    className={cn(
      'flex min-h-10 shrink-0 items-center justify-between border-border border-b-2 bg-primary px-4 text-primary-foreground',
      className,
    )}
    {...props}>
    {children}
...
const DialogBody = ({ className, ref, ...props }: ComponentProps<'div'>) => (
  <div ref={ref} className={cn('min-h-0 min-w-0 flex-1 overflow-y-auto px-4 py-3', className)} {...props} />
);

const DialogFooter = ({ className, ref, ...props }: ComponentProps<'div'>) => (
  <div
    ref={ref}
    className={cn('flex shrink-0 items-center justify-end gap-2 border-border border-t-2 px-4 py-2', className)}
    {...props}
  />
);
